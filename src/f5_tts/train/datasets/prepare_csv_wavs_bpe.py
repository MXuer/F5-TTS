...
def prepare_csv_wavs_dir(input_dir, num_workers=None, tokenizer="pinyin"):
    global executor
...
def read_audio_text_pairs(csv_file_path):
    ...
    with open(csv_file_path, mode="r", newline="", encoding="utf-8-sig") as csvfile:
        ...
        for row in reader:
            if len(row) >= 2:
                audio_file_path = Path(row[0].strip())  # First column: audio file path
...
def prepare_and_save_set(inp_dir, out_dir, is_finetune: bool = True, num_workers: int = None, tokenizer: str = 'pinyin'):
    if is_finetune:
...
Examples:
    ...
    python prepare_csv_wavs.py /input/dataset/path /output/dataset/path --workers 4
        ...
        parser.add_argument("--tokenizer", type=str, help="Tokenizer type (default: pinyin)", default='pinyin')
